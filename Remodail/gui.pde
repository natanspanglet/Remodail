...
public void createGUI(){
  ...
  hoursLabel = new GLabel(timeWindow, 66, 43, 180, 20);
  ...
  dateLabel = new GLabel(timeWindow, 50, 151, 214, 20);
  dateLabel.setTextAlign(GAlign.CENTER, GAlign.MIDDLE);
  dateLabel.setText("What is the date? (DD/MM/YYYY)");
  dateLabel.setOpaque(false);
  hoursInput = new GTextField(timeWindow, 98, 107, 120, 30, G4P.SCROLLBARS_NONE);
  ...
  howToTime = new GLabel(timeWindow, 51, 72, 204, 20);
  howToTime.setTextAlign(GAlign.CENTER, GAlign.MIDDLE);
  howToTime.setText("Write in 24 hour time! ex)19:00");
  howToTime.setOpaque(false);
  userControl.loop();
  displayControl.loop();
